...
async function responseErrorInterceptor(error, instance) {
     ...
     */
    if (status === constants_1.HTTP_ACCEPTED) {
        await utilities_1.sleep(this.retryTimeout);
...
const getInstance = (url) => {
    ...
    instance.interceptors.response.use(responseInterceptor, (error) => responseErrorInterceptor(error, instance));
    return instance;
};
...
class YooKassaRequest {
     ...
     */
    createWebHook(data, idempotenceKey) {
